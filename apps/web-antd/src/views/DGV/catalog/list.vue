    ...
    >
      <template #bodyCell="{ column, record: _record }">
        <template v-if="column.key === 'tableType'">
          <Tag
            :color="(_record as any).tableType === 'table' ? 'blue' : 'green'"
            >{{ (_record as any).tableType }}</Tag
          >
        </template>
        <template v-if="column.key === 'action'">
          <Space>
            <Button
              type="link"
              size="small"
              @click="router.push(`/DGV/catalog/detail/${(_record as any).id}`)"
              >详情</Button
            >
            <Button
              type="link"
              size="small"
              danger
              @click="handleDelete(_record as MetaTable)"
              >删除</Button
            >
          </Space>
        </template>
