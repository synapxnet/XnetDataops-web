...
<template>
  ...
  >
    ...
    <Descriptions bordered v-if="metaTable" :column="2" class="mb-4">
      ...
      <DescriptionsItem label="最后同步">{{
        metaTable.lastSyncAt
      }}</DescriptionsItem>
    ...
    <div class="mb-4" v-if="tags.length > 0">
      <h4>标签:</h4>
      <Space>
        <Tag v-for="tag in tags" :key="tag.id" :color="tag.color">{{
          tag.name
        }}</Tag>
      </Space>
    </div>
    ...
    >
      <template #bodyCell="{ column, record: _record }">
        <template v-if="column.key === 'isNullable'">
          <Tag :color="(_record as any).isNullable ? 'green' : 'red'">{{
            (_record as any).isNullable ? '是' : '否'
          }}</Tag>
        </template>
