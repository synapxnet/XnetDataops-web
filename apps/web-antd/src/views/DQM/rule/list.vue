...
<template>
  ...
  >
    <template #extra
      ><Button type="primary" @click="showCreate">新建规则</Button></template
    >
    <Table
      :columns="columns"
      ...
      <template #bodyCell="{ column, record: _record }">
        <template v-if="column.key === 'ruleType'">
          <Tag
            ...
            >{{ (_record as any).ruleType }}</Tag
          >
        </template>
        <template v-if="column.key === 'severity'">
          <Tag
            :color="severityColorMap[(_record as any).severity] || 'default'"
            >{{ (_record as any).severity }}</Tag
          >
        </template>
        ...
        <template v-if="column.key === 'action'">
          <Space>
            <Button
              type="link"
              size="small"
              @click="showEdit(_record as QualityRule)"
              >编辑</Button
            >
            <Button
              type="link"
              size="small"
              danger
              @click="handleDelete(_record as QualityRule)"
              >删除</Button
            >
          </Space>
        </template>
    ...
    >
      ...
      >
        ...
        <FormItem label="规则表达式"
          ><Textarea v-model:value="formState.ruleExpression" :rows="2"
        /></FormItem>
        <FormItem label="严重级别">
