...
const totalAssets = computed(() =>
  stats.value ? assets.value.length : undefined,
);
const coreCount = computed(
  () => assets.value.filter((a) => a.category === '核心').length,
);
const importantCount = computed(
  () => assets.value.filter((a) => a.category === '重要').length,
);
const normalCount = computed(
  () => assets.value.filter((a) => a.category === '一般').length,
);
...
const domainData = computed(() => {
  ...
  return Object.entries(stats.value.byDomain).map(([domain, count]) => ({
    domain,
    count,
  }));
});
...
const accessLevelData = computed(() => {
  ...
  return Object.entries(stats.value.byAccessLevel).map(
    ([accessLevel, count]) => ({ accessLevel, count }),
  );
});
...
<template>
  <DataPage description="发现可用资产，整理分类与使用信息。" title="资产统计">
          ...
          >
            <template #bodyCell="{ column, record: _record }">
              <template v-if="column.key === 'domain'">
                <Tag
                  :color="domainColorMap[(_record as any).domain] || 'default'"
                  >{{ (_record as any).domain }}</Tag
                >
              </template>
            </template>
          </Table>
          ...
          >
            <template #bodyCell="{ column, record: _record }">
              <template v-if="column.key === 'accessLevel'">
                <Tag
                  :color="
                    accessLevelColorMap[(_record as any).accessLevel] ||
                    'default'
                  "
