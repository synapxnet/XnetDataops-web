    ...
    >
      ...
      <FormItem label="资产类型" required>
        <Select v-model:value="formState.assetType">
          <SelectOption v-for="t in assetTypes" :key="t" :value="t">{{
            t
          }}</SelectOption>
        </Select>
      </FormItem>
      <FormItem label="业务域" required>
        <Select v-model:value="formState.domain">
          <SelectOption v-for="d in domains" :key="d" :value="d">{{
            d
          }}</SelectOption>
        </Select>
      </FormItem>
      <FormItem label="分类" required>
        <Select v-model:value="formState.category">
          <SelectOption v-for="c in categories" :key="c" :value="c">{{
            c
          }}</SelectOption>
        </Select>
      </FormItem>
      <FormItem label="访问级别">
        <Select v-model:value="formState.accessLevel">
          <SelectOption v-for="a in accessLevels" :key="a" :value="a">{{
            a
          }}</SelectOption>
        </Select>
      </FormItem>
      ...
      <FormItem label="行数">
        <InputNumber
          v-model:value="formState.rowCount"
          :min="0"
          style="width: 100%"
        />
      </FormItem>
      <FormItem label="数据大小(Bytes)">
        <InputNumber
          v-model:value="formState.dataSizeBytes"
          :min="0"
          style="width: 100%"
        />
      </FormItem>
